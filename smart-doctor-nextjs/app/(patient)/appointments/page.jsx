      ...
      <div>
        {appointments.map((item, index) => (
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
            <div>
              <Image className='w-32 bg-indigo-100 rounded-lg' src={item.docData.image} alt={item.docData.name} width={128} height={128} />
            </div>
            ...
            <div className='flex flex-col gap-2 justify-end'>
              {!item.cancelled && item.payment && !item.isCompleted && (
                <button className='sm:min-w-48 py-2 border rounded-full text-stone-500 bg-indigo-100'>Paid</button>
              )}
              {!item.cancelled && !item.payment && !item.isCompleted && (
                <button 
              ...
              {item.cancelled && !item.isCompleted && (
                <button className='sm:min-w-48 py-2 border border-red-500 text-red-500 rounded-full'>Appointment cancelled</button>
              )}
              {item.isCompleted && (
                <button className='sm:min-w-48 py-2 border border-green-500 text-green-500 rounded-full'>Completed</button>
              )}
            </div>
          </div>
        ))}
